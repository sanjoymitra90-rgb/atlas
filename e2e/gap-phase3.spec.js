...
test.describe('Gap Analyzer — Phase 3 (event pairing)', () => {
  ...
  test('pairing summary at default 1000ms window', async ({ page }) => {
    ...
    await expect(page.getByTestId('gap-pair-unverified')).toHaveText('3');
  ...
  test('correlation line crosses pairing with UK validity', async ({ page }) => {
    ...
    await expect(corr).toContainText('Of 3 signed-but-not-verified, 1 had an invalid destination');
    await expect(corr).toContainText('of 3 pairs, 1 had an invalid destination');
  });

  test('pair status pills: 6 paired, 3 unverified, 2 unsigned, 1 unpairable', async ({ page }) => {
  ...
  test('retry is last-in-wins: one unverified, two paired among the three retry rows', async ({ page }) => {
    const retryRows = page.locator('[data-testid="gap-table"] tbody tr', { hasText: '447911000105' });
    await expect(retryRows.locator('[data-pair-status="unverified"]')).toHaveCount(1);
  ...
  test('clicking signed-but-not-verified filters the table to those 3 rows', async ({ page }) => {
    await page.getByTestId('gap-pair-unverified').click();
    const rows = page.locator('[data-testid="gap-table"] tbody tr');
    await expect(rows).toHaveCount(3);
    await expect(rows.locator('[data-pair-status="unverified"]')).toHaveCount(3);
  });
