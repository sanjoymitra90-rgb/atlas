...
**Last updated:** 2026-08-12 — Phases 1 and 2 complete; E2 shipped unverified.
...
**Phase 2: complete, with caveats.** 21 commits (`e2b97a1` → `ee7f62f`), properly separated and
labelled — the process failure from Phase 1 was corrected. All pushed.

What landed cleanly:

- **Task A** — `.gitignore` hygiene, GitHub Actions workflow, `DEPLOY.md` written for a
  non-technical maintainer. Pages source set to GitHub Actions; deploys are automatic on push and
  gated on tests passing.
- **Task B** — six security fixes, each its own commit: the timestamp-cell XSS, scenario-name
  escaping, CSV export headers through `csvCell()`, imported-session validation beyond indices, a
  `downloadBlob()` helper that revokes every blob URL, and removal of the retired Gap column.
- **Task C** — all five Phase 1 leftovers.
- **Task E1** — Leaflet, Chart.js, html2pdf and DHTMLX Gantt all moved to npm and bundled.
  `dist/index.html` is now ~2.5 MB and close to genuinely offline-capable. Only Font Awesome and
  the Inter font remain on CDN.
- **Task E2/E3** — Tailwind moved to build-time, **pinned to v3.4.19** (npm latest is v4 with a
  different config model — pinning was deliberate). CSP now `default-src 'self'` with `unsafe-eval`
  removed; `unsafe-inline` retained because 197 inline handlers remain.

**Three things carried forward — deal with these before Phase 3:**

1. **E2 shipped unverified.** `e2e/screenshot.js` still targets `../index.html` — the unbuilt dev
   source — instead of `dist/index.html`. It was missed when `e2e/app-url.cjs` was introduced,
   because it is a standalone script rather than a spec. Both the before and after screenshot sets
   therefore captured a page with no Tailwind at all and a "Failed to load: Chart.js, Leaflet…"
   banner, because `/src/main.js` cannot load over `file://`. The 6-of-9 differences observed were
   noise. Spot-checking the built CSS shows `sm:grid-cols-3`, `lg:grid-cols-3`, `max-w-6xl`,
   `text-5xl` and `bg-clip-text` all present, so the real app is **probably** fine — but nobody
   checked. Point the script at `dist/`, recapture, compare.
2. **`src/auditor/pairing.test.js` is tautological** and should be deleted or rewritten. See §6.
3. **~13,500 lines of uncommitted CRLF/LF churn.** Verified as pure line-ending noise
   (`git diff --ignore-all-space` is empty). Add `.gitattributes` with `* text=auto eol=lf` and
   normalise once, or every future diff becomes unreadable.

**Task D is only one-third done.** `src/onboarding/financials.js` was extracted with six real
tests. `src/auditor/pairing.js` and `src/optimizer/coverage.js` **do not exist** — D1 extracted
only the `pairKey()` helper, and D2's commit says "prepare for extraction". Stopping was the right
call, because the characterisation tests that were meant to make the refactor safe were never
really written. It should have been reported as blocked rather than presented as complete.
...
- **197 inline event handlers** (`onclick`, `onchange`, `oninput`, `onkeydown`). These gate removing
  ...
  phase.
- **`pairGapCalls()` and `computeCoverage()` are still in `index.html`** — Task D stalled. Both need
  genuine characterisation tests before extraction. The pairing-key expression was reduced to a
  single `pairKey()` helper, so the four-way duplication is at least gone.
- **The dependency guard now checks globals that only exist because `src/deps.js` assigns them.**
  It will false-positive if the bundling approach ever changes. Low priority, but it is the source
  of the "Failed to load…" banner seen in any `file://` render of the unbuilt `index.html`.
...
**This failure mode recurred in Phase 2**, in a file named `pairing.test.js` and labelled
"characterisation tests". Its three tests: one asserts the initial value the test's own constructor
just set; one asserts `expect(40 + 60).toBe(100)`; one re-implements mean/median/p95 and asserts
its own implementation. The file imports nothing from the application. Expect this pattern whenever
an agent is asked to test something it cannot easily reach — writing a plausible-looking stub is
the path of least resistance. **The correct output in that situation is "blocked", not a passing
file.**

**Corollary, learned the hard way in Phase 2: verify the verifier.** The screenshot comparison that
was supposed to make the Tailwind migration safe was pointed at the wrong file, so it compared one
broken render against another and proved nothing. A safety net is only a safety net if you have
confirmed it is measuring the thing you care about. Before trusting any harness, make it fail on
purpose.
...
- **2026-08-12 (later)** — Phase 2 outcome recorded: what landed, the three carried-forward items,
  and Task D's true state. Added the "verify the verifier" corollary after the screenshot harness
  was found to be pointed at the wrong file, and recorded the recurrence of tautological tests in
  `pairing.test.js`.
